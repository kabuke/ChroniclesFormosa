...
| :--- | :---       | :---     |
| 1.0  | 2026-03-30 | 初版建立。 |
| 1.1  | 2026-04-15 | Step 1 i18n：補齊天災/救災/預警/獎勵 25 個 key，更新四語系 seedAll()，重構 UI 層。 |
| 1.2  | 2026-04-15 | Step 2 音效：產生 sfx_earthquake/typhoon/warning/relief_success/relief_fail.wav，接入 PlaySFX。 |
| 1.3  | 2026-04-16 | Step 3 單元測試：新增 timer/earthquake/typhoon/relief/interaction/handler 六組測試，`go test ./server/...` 全綠。 |
...
- [x] 4.1 天災名稱與描述文字
- [x] 4.2 預警訊息模板（i18n Key + `KEY|PARAM` 格式）
- [x] 4.3 救災小遊戲 UI 文字
- [x] 4.4 獎勵描述文字（PERFECT/GOOD/FAIL 三級）
- [x] 4.5 隱藏資源發現文字
...
- [x] 5.1 地震音效（低頻隆隆聲 `sfx_earthquake.wav`）
- [x] 5.2 颱風音效（風雨聲 `sfx_typhoon.wav`）
- [x] 5.3 預警音效（鐘聲 `sfx_warning.wav`）
- [x] 5.4 救災成功音效（`sfx_relief_success.wav`）
- [x] 5.5 救災失敗音效（`sfx_relief_fail.wav`）

---

## 六、測試

- [x] 6.1 天災排程器頻率測試（`timer_test.go`，模擬 259200 tick）
- [x] 6.2 地震傷害計算單元測試（`earthquake_test.go`，包含 5 段 table test）
- [x] 6.3 颱風路徑生成測試（`typhoon_test.go`，受災數範圍 + 邊界條件）
- [x] 6.4 救災路線評分演算法單元測試（`relief_test.go`，PERFECT/GOOD/FAIL）
- [x] 6.5 獎勵計算測試（DB 驗證 Food + Loyalty 增減）
- [x] 6.6 天災 + 戰鬥交互測試（`interaction_test.go`，高 TensionValue 地震場景）
- [ ] 6.7 小遊戲端對端流程測試（需手動 QA，非自動化）
