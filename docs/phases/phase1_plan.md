...
1. [x] `go build ./server` 編譯無誤 (伺服器端完成)
2. [ ] `go build ./client` 均無錯誤 (準備進入開發)
3. [ ] 客戶端啟動後可連線至伺服器並完成登入握手
4. [ ] 大地圖場景可流暢滾動，視野邊緣 Chunk 自動載入
5. [ ] Navbar 正確顯示連線延遲
6. [ ] Toast 可顯示成功/錯誤訊息
7. [ ] i18n 切換語系時所有 UI 文字正確更新
8. [x] Server 單元模組/手動測試驗證通過（Logic 層、AOI分流、Village資源產出）
